    ...
    public function list(Request $request)
{
    ...
    $mhs = MahasiswaKompenModel::with('mahasiswa') // Memuat relasi mahasiswa
    ...
    return DataTables::of($mhs)
        ...
        ->addColumn('aksi', function ($mhs) {
            $id = $mhs->absensi_id; // Primary key dari MahasiswaKompenModel
            return '
                <button onclick="modalAction(\'' . url('mhs.show_ajax', $id) . '\')" class="btn btn-info btn-sm">Detail</button>
    ...
    {
        $mhs = MahasiswaKompenModel::with('mahasiswa')->find($id); // Load relasi mahasiswa
    ...
    public function store_ajax(Request $request)
    {
        if ($request->ajax() || $request->wantsJson()) {
            ...
            MahasiswaKompenModel::create($request->all());
    ...
    {
        $mhs = MahasiswaKompenModel::find($id);
    ...
    public function update_ajax(Request $request, $id)
    {
        if ($request->ajax() || $request->wantsJson()) {
            ...
            $check = MahasiswaKompenModel::find($id);
    ...
    public function import_ajax(Request $request)
    {
        if ($request->ajax() || $request->wantsJson()) {
            ...
            if (count($data) > 1) {
                ...
                if (count($insert) > 0) {
                    MahasiswaKompenModel::insertOrIgnore($insert);
                }
    ...
    public function export_excel()
    {
        ...
        $mhs = MahasiswaKompenModel::select('mahasiswa_id', 'sakit', 'izin', 'alpha', 'poin', 'status', 'periode')
